...
public extension MockDependencyGraph {
    init(_ json: JSON) {
        ...
        self.result = Dictionary(uniqueKeysWithValues: result.map { value in
            ...
            let package = PackageReference(identity: PackageIdentity2(container.lowercased()), path: "/\(container)", kind: .remote)
...
private extension MockPackageContainer.Constraint {
    init(json: JSON) {
        ...
        let package = PackageReference(identity: PackageIdentity2(identifier), path: "", kind: .remote)
