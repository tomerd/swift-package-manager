...
public extension MockDependencyGraph {
    init(_ json: JSON) {
        ...
        self.result = Dictionary(uniqueKeysWithValues: result.map { value in
            ...
            let package = PackageReference(identity: PackageIdentity2(container.lowercased()), kind: .remote, path: "/\(container)")
...
private extension MockPackageContainer.Constraint {
    init(json: JSON) {
        ...
        let package = PackageReference(identity: PackageIdentity2(identifier), kind: .remote, path: "")
