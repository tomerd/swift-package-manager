...
public class RepositoryPackageContainer: PackageContainer, CustomStringConvertible {
    ...
    public func getDependencies(at version: Version, productFilter: ProductFilter) throws -> [Constraint] {
        do {
            ...
            throw GetDependenciesError(
                containerIdentifier: self.underlying.repository.url, reference: version.description, underlyingError: error, suggestion: nil)
        }
    }
        ...
        do {
            ...
            if let error = error as? GitRepositoryError, error.description.contains("Needed a single revision") {
                ...
                if let rev = try? repository.resolveRevision(identifier: revision), repository.exists(revision: rev) {
                    ...
                    throw GetDependenciesError(
                        containerIdentifier: self.underlying.repository.url, reference: revision, underlyingError: error, suggestion: nil)
                }
                else {
                    ...
                    throw GetDependenciesError(
                        containerIdentifier: self.underlying.repository.url, reference: revision,
            ...
            throw GetDependenciesError(containerIdentifier: self.underlying.repository.url, reference: revision, underlyingError: error, suggestion: nil)
        }
    }
    ...
    private func loadManifest(at revision: Revision, version: Version?) throws -> Manifest {
        try self.manifestsCache.memoize(revision) {
            let fs = try repository.openFileView(revision: revision)
            let packageURL = self.underlying.repository.url

            // Load the tools version.
            let toolsVersion = try toolsVersionLoader.load(at: .root, fileSystem: fs)

            // Validate the tools version.
            try toolsVersion.validateToolsVersion(self.currentToolsVersion, version: revision.identifier, packagePath: packageURL)
    ...
    public var description: String {
        return "RepositoryPackageContainer(\(self.underlying.repository.url.debugDescription))"
    }
}
