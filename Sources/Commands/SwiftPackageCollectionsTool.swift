...
public struct SwiftPackageCollectionsTool: ParsableCommand {
    ...
    struct Describe: ParsableCommand {
        ...
        mutating func run() throws {
            // FIXME: this should transition to identity instead of URL
            let identity = PackageIdentity2(packageUrl)
            let reference = PackageReference(identity: identity, path: packageUrl, kind: .remote)
