...
private class MockRepository: Repository {
    ...
    var packageRef: PackageReference {
        return PackageReference.remote(identity: PackageIdentity(url: self.url), url: self.url)
    }
...
class RepositoryPackageContainerProviderTests: XCTestCase {
    func testVprefixVersions() throws {
        ...
        let ref = PackageReference.remote(identity: PackageIdentity(path: repoPath), url: repoPath.pathString)
    ...
    func testVersions() throws {
        ...
        do {
            ...
            let ref = PackageReference.remote(identity: PackageIdentity(url: specifier.url), url: specifier.url)
            let container = try provider.getContainer(for: ref, skipUpdate: false)
            let v = try container.toolsVersionsAppropriateVersionsDescending().map { $0 }
    ...
    func testMissingBranchDiagnostics() throws {
        try testWithTemporaryDirectory { tmpDir in
            ...
            let packageRef = PackageReference.remote(identity: PackageIdentity(path: packageDir), url: packageDir.pathString)
    ...
    func testRepositoryPackageContainerCache() throws {
        ...
        try testWithTemporaryDirectory { temporaryDirectory in
            ...
            let packageReference = PackageReference.remote(identity: PackageIdentity(path: packageDirectory), url: packageDirectory.pathString)
