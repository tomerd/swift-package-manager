...
class ManifestSourceGenerationTests: XCTestCase {
    ...
    private func testManifestWritingRoundTrip(manifestContents: String, toolsVersion: ToolsVersion, fs: FileSystem = localFileSystem) throws {
        try withTemporaryDirectory { packageDir in
            ...
            let manifest = try manifestLoader.load(at: packageDir, kind: .root, baseURL: packageDir.pathString, toolsVersion: toolsVersion)
            ...
            print(newContents)
            let newManifest = try manifestLoader.load(at: packageDir, kind: .root, baseURL: packageDir.pathString, toolsVersion: toolsVersion)
