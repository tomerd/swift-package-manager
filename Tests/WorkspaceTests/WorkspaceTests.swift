...
final class WorkspaceTests: XCTestCase {
    ...
    func testSimpleAPI() throws {
        ...
        let manifest = try tsc_await { ManifestLoader.loadManifest(
            packagePath: package, swiftCompiler: swiftCompiler, swiftCompilerFlags: [], packageKind: .local, on: .global(), completion: $0
        ) }
        let loadedPackage = try tsc_await { PackageBuilder.loadPackage(
            packagePath: package, swiftCompiler: swiftCompiler, swiftCompilerFlags: [], xcTestMinimumDeploymentTargets: [:], diagnostics: diagnostics, on: .global(), completion: $0
        ) }
